...
int main(int argc, char **argv)
{
    ...
    */
    ...
    if (!strcmp(argv[1], "/dev/ttyS1"))
    {
        get_send_confirm(fd, oldtio);
    }

    if (!strcmp(argv[1], "/dev/ttyS0"))
    {
        printf("entrou read\n");
...
void read_respond(int fd, struct termios oldtio)
{
    ...
    while (STOP == FALSE)
    {                                         /* loop for input */
        int res = read(fd, buf, 8); /* returns after 1 char have been input */
        strcat(aux, buf);

        printf("%s   %d byte read\n", buf, res); /* so we can printf... */
